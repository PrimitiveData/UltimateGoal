...
public class AutoAim2 extends Thread {
    Hardware hardware;

    Telemetry telemetry;
    LinearOpMode parentOP;
    public boolean stopRequested;
    public AutoAim2(Hardware hardware, Telemetry telemetry, LinearOpMode parentOP){
    ...
    public void run(){
        while(!parentOP.isStopRequested()&&!stopRequested) {
            ...
            hardware.turret.turretPID.setState(MathFunctions.keepAngleWithinSetRange(-Math.toRadians(240),Math.toRadians(120),angleToGoal-hardware.angle));
            telemetry.update();
        }
    }
}
