...
public class UltimateGoalTeleop extends OpMode {
    ...
    public void loop(){
        ...
        if(gamepad1.dpad_left){
            ...
            hardware.turret.updatePID = true;
            hardware.turret.turretPID.setState(Math.toRadians(-179.5) - MathFunctions.keepAngleWithin180Degrees(hardware.angle));
            sleeep(1000);
            shootPowershot(hardware);
            telemetry.addLine("1st powershot");
            telemetry.update();
            hardware.turret.turretPID.setState(Math.toRadians(-184) - MathFunctions.keepAngleWithin180Degrees(hardware.angle));
            sleeep(900);
            shootPowershot(hardware);
            telemetry.addLine("2nd powershot");
            telemetry.update();
            hardware.turret.turretPID.setState(Math.toRadians(-189.5) - MathFunctions.keepAngleWithin180Degrees(hardware.angle));
            sleeep(900);
            shootPowershot(hardware);
