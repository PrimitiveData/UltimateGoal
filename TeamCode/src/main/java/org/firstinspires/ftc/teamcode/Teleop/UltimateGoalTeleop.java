...
public class UltimateGoalTeleop extends OpMode {
    ...
    public void loop(){
        ...
        if(gamepad1.dpad_left){
            ...
            hardware.turret.updatePID = true;
            hardware.turret.turretPID.setState(MathFunctions.keepAngleWithin180Degrees(Math.toRadians(-179.5) - hardware.angle));
            sleeep(1000);
            shootPowershot(hardware);
            telemetry.addLine("1st powershot");
            telemetry.update();
            hardware.turret.turretPID.setState(MathFunctions.keepAngleWithin180Degrees(Math.toRadians(-184) - hardware.angle));
            sleeep(900);
            shootPowershot(hardware);
            telemetry.addLine("2nd powershot");
            telemetry.update();
            hardware.turret.turretPID.setState(MathFunctions.keepAngleWithin180Degrees(Math.toRadians(-189.5) - hardware.angle));
            sleeep(900);
            shootPowershot(hardware);
